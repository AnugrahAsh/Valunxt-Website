...
class VamtamEnqueues {
	...
	public static function actions() {
		...
		add_action( 'enqueue_block_assets', array( __CLASS__, 'block_editor_variables' ) );
	...
	public static function scripts() {
		...
		if ( current_user_can( 'edit_posts' ) &&
			...
			$elementor_registered = false;

			if ( function_exists( 'wp_script_is' ) ) {
				$elementor_registered = wp_script_is( 'elementor-app-loader', 'registered' );
			}

			if ( ! $elementor_registered && isset( $GLOBALS['wp_scripts'] ) ) {
				$elementor_registered = (bool) $GLOBALS['wp_scripts']->query( 'elementor-app-loader', 'registered' );
			}

			if ( $elementor_registered ) {
	...
	public static function admin_scripts() {
		...
		wp_localize_script(
			'vamtam-admin', 'VAMTAM_ADMIN', array(
				'addNewIcon' => esc_html__( 'Add New Icon', 'execor' ),
				'iconName'   => esc_html__( 'Icon', 'execor' ),
				'iconText'   => esc_html__( 'Text', 'execor' ),
				'iconLink'   => esc_html__( 'Link', 'execor' ),
				'iconChange' => esc_html__( 'Change', 'execor' ),
				'fonts'      => $GLOBALS['vamtam_fonts'],
				'home_url'   => home_url(),
				'ownStoreTxt' => esc_html__( 'Enter your purchase code from vamtam.com to receive theme updates and support.', 'execor' ),
				'elementsTxt' => esc_html__( 'Enter your Envato Elements Token.', 'execor' ),
				'tfPcTxt' => esc_html__( 'Enter your purchase code from ThemeForest to receive theme updates and support.', 'execor' ),
				'unRegOwnStoreTxt' => esc_html__( 'You are about to unregister your vamtam.com purchase code. Are you sure you want to continue?', 'execor' ),
				'unRegPcTxt' => esc_html__( 'You are about to unregister your purchase code. Are you sure you want to continue?', 'execor' ),
				'unRegTokenTxt' => esc_html__( 'You are about to unregister your Envato Elements Token. Are you sure you want to continue?', 'execor' ),
				'unRegInvalidTokenTxt' => esc_html__( 'There seems to be an issue with your current token registration, likely due to site migration. If you proceed, the theme will be unregistered at your end but the token will remain bound to the URL it was registered at, until you unregister it from there. If you\'d like to re-register this as an Envato Elements purchase, you can always generate a new token. Are you sure you want to continue?', 'execor' ),
		...
		if ( $pagenow === 'update-core.php' ) {
			...
			<script type="text/javascript">
				document.addEventListener( 'DOMContentLoaded', function() {
					...
					if ( ! $valid_pc ) {
						$update_notice  = '<br id="vamtam-notice-br" />' . __( 'Note: Only Envato Market clients with a valid purchase code are entitled to automatic updates. Envato Elements clients must use FTP.', 'execor' );
						$changelog_link = '<span><a href="https://vamtam.com/changelog" target="_blank">' . __('View Changelog', 'execor') . '</a></span>';
	...
	public static function handle_dismiss_admin_notice() {
		...
		// dismiss the notice for several days
		set_transient( $option_name, true, 7 * DAY_IN_SECONDS );
	...
	/**
	 * Elementor's global colours/typography (--e-global-*) as CSS variables
	 */
	public static function get_block_editor_variables_css() {
		...
		$globals  = '';
		$kit_css  = \Elementor\Core\Files\CSS\Post::create( $kit_id )->get_content();
		$selector = '.elementor-kit-' . (int) $kit_id;

		if ( $kit_css && preg_match_all(
			'/(@media[^{]+\{)?\s*' . preg_quote( $selector, '/' ) . '\s*\{([^{}]*--e-global[^{}]*)\}/',
			$kit_css,
			$blocks,
			PREG_SET_ORDER
		) ) {
			foreach ( $blocks as $block ) {
				if ( ! preg_match_all( '/--e-global-(?:color|typography)-[\w-]+\s*:[^;}]+;/', $block[2], $decls ) ) {
					continue;
				}

				$media   = trim( $block[1] );
				$rule    = ':root{' . implode( '', $decls[0] ) . '}';
				$globals .= $media ? $media . $rule . '}' : $rule;
			}
		}
	...
	/**
	 * Inject Elementor's global colours/typography into the block editor so the
	 * --vamtam-* aliases compiled into editor.css can resolve them.
	 */
	public static function block_editor_variables() {
		if ( ! is_admin() ) {
			return;
		}

		$css = self::get_block_editor_variables_css();

		if ( '' === $css ) {
			return;
		}

		wp_register_style( 'vamtam-editor-variables', false, [], VamtamFramework::get_version() );
		wp_enqueue_style( 'vamtam-editor-variables' );
		wp_add_inline_style( 'vamtam-editor-variables', $css );
	}
