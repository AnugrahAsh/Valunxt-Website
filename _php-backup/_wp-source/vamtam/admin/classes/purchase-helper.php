...
class VamtamPurchaseHelper extends VamtamAjax {
	...
	private static function server_tests() {
		...
		$tests = array(
			array(
				'name'  => esc_html__( 'PHP Version', 'execor' ),
				'test'  => version_compare( phpversion(), '5.5', '<' ),
				'value' => phpversion(),
				'desc'  => esc_html__( 'While this theme works with all PHP versions supported by WordPress Core, PHP versions 5.5 and older are no longer maintained by their developers. Consider switching your server to PHP 5.6 or newer.', 'execor' ),
			),
			array(
				'name'  => esc_html__( 'PHP Time Limit', 'execor' ),
				'test'  => $timeout > 0 && $timeout < 30,
				'value' => $timeout,
				'desc'  => esc_html__( 'The PHP time limit should be at least 30 seconds. Note that in some configurations your server (Apache/nginx) may have a separate time limit. Please consult with your hosting provider if you get a time out while importing the demo content.', 'execor' ),
			),
			array(
				'name'  => esc_html__( 'PHP Memory Limit', 'execor' ),
				'test'  => (int) $memory > 0 && $memoryB < 96 * 1024 * 1024,
				'value' => $memory,
				'desc'  => esc_html__( 'You need a minimum of 96MB memory to use the theme and the bundled plugins. For non-US English websites you need a minimum of 128MB in order to accomodate the translation features which are otherwise disabled.', 'execor' ),
			),
			array(
				'name'  => esc_html__( 'PHP ZipArchive Extension', 'execor' ),
				'test'  => ! class_exists( 'ZipArchive' ),
				'value' => '',
				'desc'  => esc_html__( 'ZipArchive is a requirement for importing the demo sliders.', 'execor' ),
			),
		);
	...
	private static function should_show_own_store_tab() {
		if ( defined( 'VAMTAM_SHOW_OWN_STORE' ) && VAMTAM_SHOW_OWN_STORE ) {
			return true;
		}

		$timezone = new DateTimeZone( 'America/New_York' );
		$now = new DateTime( 'now', $timezone );
		$cutoff = new DateTime( '2026-07-01 00:00:00', $timezone );
	...
	public static function tgmpa_plugins_complete_actions( $update_actions, $plugin_info ) {
		if ( isset( $update_actions['dashboard'] ) ) {
			$update_actions['dashboard'] = sprintf(
				esc_html__( 'All plugins installed and activated successfully. %1$s', 'execor' ),
				'<a href="' . esc_url( admin_url( 'admin.php?page=vamtam_theme_setup_import_content' ) ) . '" class="button button-primary">' . esc_html__( 'Continue with theme setup.', 'execor' ) . '</a>'
			);
	...
	public static function admin_menu() {
		add_menu_page( esc_html__( 'VamTam', 'execor' ), esc_html__( 'VamTam', 'execor' ), 'edit_theme_options', 'vamtam_theme_setup', array( __CLASS__, 'page' ), '', 2 );
		add_submenu_page( 'vamtam_theme_setup', esc_html__( 'Dashboard', 'execor' ), esc_html__( 'Dashboard', 'execor' ), 'edit_theme_options', 'vamtam_theme_setup', array( __CLASS__, 'page' ) );
	...
	public static function admin_menu_1() {
		...
		add_submenu_page( 'vamtam_theme_setup', esc_html__( 'Import Demo Content', 'execor' ), esc_html__( 'Import Demo Content', 'execor' ), 'edit_theme_options', 'vamtam_theme_setup_import_content', array( __CLASS__, 'vamtam_theme_setup_import_content' ) );
	}

	public static function admin_menu_2() {
		add_submenu_page(
			'vamtam_theme_setup',
			esc_html__( 'Services', 'execor' ),
			esc_html__( 'Deals Up To', 'execor' ) .
			'<span id="vamtam-premium-services">' . esc_html__( '70% Off', 'execor' ) . '</span>',
			'edit_theme_options',
	...
	private static function should_show_offers_notice() {
		...
		if ( false !== strpos( (string) $screen->id, 'execor' ) || $screen->is_block_editor() ) {
			return false;
		}
	...
	public static function registration_warning() {
		...
		<div class="vamtam-notice-wrap">
			<div class="vamtam-notice">
				<p>
					<?php echo esc_html__( 'Please activate your license to get theme updates, premium support, and access to demo content.', 'execor' ); ?>
					<a href="<?php echo esc_url( admin_url( 'admin.php?page=vamtam_theme_setup' ) ); ?>">
						<?php echo esc_html__( 'Register Now', 'execor' ); ?>
					</a>
				</p>
			</div>
		</div>
	...
	public static function admin_init() {
		...
		$registration_label = self::should_show_own_store_tab() ?
			esc_html__( 'Enter your purchase code from vamtam.com to receive theme updates and support.', 'execor' ) :
			esc_html__( 'Enter your purchase code from ThemeForest to receive theme updates and support.', 'execor' );
	...
	public static function page() {
		...
		$show_own_store_tab = self::should_show_own_store_tab();
		...
		<div id="vamtam-ts-homepage" class="vamtam-ts">
			...
			<div id="vamtam-ts-main">
				...
				<div id="vamtam-check-license-disclaimer">
					<h5><?php esc_html_e( 'Licensing Terms', 'execor' ); ?></h5>
					<p>
						<?php esc_html_e( 'You need to register a separate license for each domain on which you will use the theme. A single license is limited to a single domain/application. For more information, please refer to these articles - ', 'execor' ); ?>
						<a href="http://themeforest.net/licenses" target="_blank">
							<?php esc_html_e( 'Licensing Terms Envato Market', 'execor' ); ?>
						</a>,
						<a href="https://elements.envato.com/license-terms" target="_blank">
							<?php esc_html_e( 'Licensing Terms Envato Elements', 'execor' ); ?>
						</a>
						.
					</p>
				</div>
						...
						<div id="vamtam-server-tests">
							<h3>
								<?php if ( $status['fail'] > 0 ) : ?>
									<?php esc_html_e( 'System Status', 'execor' ) ?>
									<?php $fail = $status['fail']; ?>
									<small><?php printf( esc_html( _n( '(%d potential issue)', '(%d potential issues)', $fail, 'execor' ) ), $fail ) ?></small>
								<?php endif ?>
							</h3>
						</div>
					<?php endif ?>
	...
	public static function dashboard_navigation() {
		...
		<nav id="vamtam-ts-nav-menu">
			<div id="vamtam-theme-title">
				<span id="vamtam-ts-greeter"><?php esc_html_e( 'WELCOME TO', 'execor' ); ?></span>
				<span id="vamtam-ts-greeter-title"><?php echo esc_html( $theme_name ); ?></span>
				<span id="vamtam-ts-greeter-ver"><?php echo sprintf( esc_html__( 'VER. %s', 'execor' ), $theme_version ); ?></span>
			</div>
			<ul>
				<li class="<?php echo esc_attr( $cur_route === 'toplevel_page_' . $routes[0] ? 'is-active' : '' ); ?>" >
					<a href="<?php echo esc_url( admin_url( 'admin.php?page=' . $routes[0] ) ); ?>">
						...
						<span><?php echo esc_html__( 'Register' , 'execor' ); ?></span>
				...
				<?php if ( isset( $tgmpa_instance ) && isset( $tgmpa_instance->page_hook ) ) : ?>
					<li class="<?php echo esc_attr( $cur_route === 'vamtam_page_' . $routes[1] ? 'is-active' : '' ); ?>" >
						<a <?php echo esc_attr( ! $valid_key ? 'class=disabled' : '' ); ?> href="<?php echo esc_url( admin_url( 'admin.php?page=' . $routes[1] ) ); ?>">
							...
							<span><?php echo esc_html__( 'Install Plugins' , 'execor' ); ?></span>
				...
				<li class="<?php echo esc_attr( $cur_route === 'vamtam_page_' . $routes[2] ? 'is-active' : '' ); ?>" >
					<a <?php echo esc_attr( ! $valid_key || $plugin_status !== 'success' ? 'class=disabled' : '' ); ?> href="<?php echo esc_url( admin_url( 'admin.php?page=' . $routes[2] ) ); ?>">
						...
						<span><?php echo esc_html__( 'Import Demo' , 'execor' ); ?></span>
	...
	public static function import_buttons() {
		...
		$messages = array(
			'success-msg' => esc_html__( 'Imported.', 'execor' ),
			'error-msg  ' => esc_html__( 'Failed to import. Please <a href="{fullimport}" target="_blank">click here</a> in order to see the full error message.', 'execor' ),
		);

		$import_tests = array(
			array(
				'test'   => defined( 'ELEMENTOR_PRO__FILE__' ),
				'title'  => esc_html__( 'Posts, Pages and Site Layout', 'execor' ),
				'failed' => wp_kses( __( "This theme requires Elementor Pro. If you don't have Elementor Pro, please <a href='https://be.elementor.com/visit/?bta=13981&nci={$el_pro_nci}' target='_blank'>download it here</a>. Install and activate it, and then proceed with importing the demo content. If you have any issues with the importer please <a href='https://elementor.support.vamtam.com/support/solutions/articles/245218-vamtam-elementor-themes-how-to-install-the-theme-via-the-admin-panel-' target='_blank'>read this article</a> or reach out to us using <a href='https://vamtam.com/contact-us/' target='_blank'>the form on this page</a>.", 'execor' ), 'vamtam-a-span' ),
			),
		);
		...
		$buttons = array(
			array(
				'label'          => esc_html__( 'Dummy Content Import', 'execor' ),
				'id'             => 'content-import-button',
				'description'    => esc_html__( 'You are advised to use this importer only on new WordPress sites.', 'execor' ),
				'button_title'   => $content_imported ? esc_html__( 'Imported', 'execor' ) : esc_html__( 'Import', 'execor' ),
				...
				'data'           => array_merge( $messages, [
					...
					'success-msg'      => sprintf( esc_html__( 'Main content imported. Image import progress: <span class="vamtam-image-import-progress">%s</span>.', 'execor' ), $img_progress ),
					'fail-msg'         => esc_html__( 'Failed to import. We recommend that you contact your hosting provider for advice, as solving this issue is often specific to each server.', 'execor' ),
					'timeout-msg'      => esc_html__( 'Failed to import. This is most likely caused by a timeout. Please contact your hosting provider for advice as to how you can increase the time limit on your server.', 'execor' ),
				] ),
	...
	public static function purchase_key( $args ) {
		...
		$placeholder = __( 'XXXXXX-XXX-XXXX-XXXX-XXXXXXXX', 'execor' );
		$plugin_status = VamtamPluginManager::get_required_plugins_status();
		$content_imported = ! ! get_option( 'vamtam_last_import_map', false );
		$show_own_store_tab = self::should_show_own_store_tab();
		...
		if ( ! defined( 'ENVATO_HOSTED_SITE' ) ) {
			...
			if ( ! $valid_key ) {
				...
				echo '<span id="vamtam-help-envato"' . ( $show_own_store_tab ? ' class="hidden"' : '' ) . '>';
				echo wp_kses( sprintf( __( ' <a href="%s" target="_blank">Where can I find my Item Purchase Code?</a>', 'execor' ), 'https://help.market.envato.com/hc/en-us/articles/202822600-Where-Is-My-Purchase-Code-' ), 'vamtam-a-span' );
				echo wp_kses( sprintf( __( ' <a href="%s" target="_blank">Cannot validate Purchase Code?</a>', 'execor' ), 'https://elementor.support.vamtam.com/support/solutions/articles/252289-cannot-validate-purchase-key-or-token' ), 'vamtam-a-span' );
				echo '</span>';
				echo '<span id="vamtam-help-own-store"' . ( ! $show_own_store_tab ? ' class="hidden"' : '' ) . '>';
				echo wp_kses( sprintf( __( 'Your vamtam.com purchase code is available in your <a href="%s" target="_blank">Order Details</a>', 'execor' ), 'https://vamtam.com/my-account/orders/' ), 'vamtam-a-span' );
				echo wp_kses( sprintf( __( ' <a href="%s" target="_blank">Cannot validate Purchase Code?</a>', 'execor' ), 'https://elementor.support.vamtam.com/support/solutions/articles/252289-cannot-validate-purchase-key-or-token' ), 'vamtam-a-span' );
				echo '</span>';
				echo '</p>';

				echo '<p id="vamtam-code-help-elements" class="hidden">';
				echo wp_kses( sprintf( __( ' <a href="%s" target="_blank">Follow this link to generate a new Envato Elements Token.</a>', 'execor' ), esc_url( 'https://api.extensions.envato.com/extensions/begin_activation'
					. '?extension_id=' . md5( get_site_url() )
					. '&extension_type=envato-wordpress'
					. '&extension_description=' . wp_get_theme()->get( 'Name' ) . ' (' . get_home_url() . ')'
					) ), 'vamtam-a-span' );
				echo wp_kses( sprintf( __( ' <a href="%s" target="_blank">Cannot validate Token?</a>', 'execor' ), 'https://elementor.support.vamtam.com/support/solutions/articles/252289-cannot-validate-purchase-key-or-token' ), 'vamtam-a-span' );
				echo '</p>';
				...
				echo '>' . esc_html__( 'Register', 'execor' );
				echo '</button>';
				...
				echo esc_html__( 'Continue to required plguins', 'execor' );
				echo '</a>';
				...
				echo esc_html__( 'Continue to demo import', 'execor' );
				echo '</a>';
			}
