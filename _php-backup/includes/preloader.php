<?php /* VALUNXT Capital intro preloader — klaygroup-style logo-mask reveal.
   ...
   The hole is cut with the VECTOR logo (paths from assets/content/uploads/
   logo/valunxt.svg — the 410x82 wordmark) inside an
   SVG <mask>, and the zoom scales the mask GEOMETRY. The previous version
...
<div id="vx-preloader" role="presentation" aria-hidden="true">
      ...
      <mask id="vxHoleMask">
        ...
        <g class="vx-hole-zoom">
          ...
          <g class="vx-hole-fit" fill="#000" shape-rendering="geometricPrecision">
            ...
            <path d="M286.538 28.593C285.281 25.5099 283.598 22.9874 281.493 21.0208C279.382 19.0542 276.92 17.6014 274.093 16.6625C271.27 15.7282 268.33 15.2564 265.282 15.2564C260.349 15.2564 255.997 16.3075 252.231 18.4049C248.466 20.5069 245.816 23.2537 244.292 26.6497H244.025V16.8633H231.917V80.3929H244.025V47.6892C244.025 44.0269 244.471 40.8551 245.371 38.1738C246.267 35.4924 247.523 33.2829 249.137 31.5405C250.75 29.7981 252.635 28.5276 254.787 27.7194C256.939 26.916 259.27 26.5142 261.783 26.5142C263.669 26.5142 265.483 26.8038 267.232 27.3878C268.982 27.967 270.529 28.9059 271.875 30.1999C273.221 31.4985 274.294 33.1942 275.101 35.2916C275.908 37.3936 276.316 39.9161 276.316 42.8637V80.3929H288.423V39.5144C288.423 35.3149 287.795 31.676 286.538 28.593Z"/>
            <path d="M327.307 59.7037L314.388 76.9454L314.275 77.0762L314.181 77.2678C314.027 77.5434 312.142 80.6778 307.232 80.6778C307.138 80.6778 307.044 80.6778 306.951 80.6731C302.993 80.5797 297.76 80.561 293.905 80.5563L320.324 45.7926L327.307 59.7037Z"/>
            <path d="M329.619 56.7187L324.517 63.4127C323.593 55.4061 318.88 50.0061 318.106 49.17L293.792 17.8864C296.118 17.8957 307.251 17.891 308.428 17.8677H308.512C311.044 17.8677 313.29 19.881 313.722 20.2921L326.228 36.7537C335.396 47.8013 330.238 55.6163 329.619 56.7187Z"/>
  ...
  <img class="vx-solid-logo" src="<?= BASE ?>/assets/content/uploads/logo/valunxt-white.svg" alt="VALUNXT Capital" width="410" height="82">
</div>
<script>
/* Align the vector hole with the solid logo. Measuring the <img> box means the
   hole inherits the CSS clamp(170px,26vw,280px) sizing exactly, so the swap
   from solid logo to window is pixel-identical at every viewport width. */
(function(){
  var el=document.getElementById('vx-preloader');
  if(!el){return;}
  ...
  function place(){
    var r=img.getBoundingClientRect(),size=r.width||410,k=size/410;
    fit.setAttribute('transform','translate('+r.left+','+r.top+') scale('+k+')');
    zoom.style.transformOrigin=(r.left+size/2)+'px '+(r.top+(r.height||size/5)/2)+'px';
  }
  place();
...
(function(){
  var el=document.getElementById('vx-preloader');
  if(!el){return;}
  if(!document.documentElement.classList.contains('vx-intro-play')){if(el.parentNode)el.parentNode.removeChild(el);return;}
  var done=false;
  function kill(){if(done)return;done=true;if(el&&el.parentNode)el.parentNode.removeChild(el);}
  el.addEventListener('animationend',function(e){if(e.animationName==='vxOverlayOut'||e.animationName==='vxFadeOnly')kill();});
