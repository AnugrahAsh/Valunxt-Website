...
$__modules = array(
    array(
        ...
        'notes' => 'Confidence bands widen automatically where comparable depth is thin. Anything below the review threshold is routed to a valuer rather than published as a number.',
    ),
    array(
        'n'     => 'Module 02',
        ...
        'notes' => 'Coverage depth varies by market. Each chart states its own data window and source so a thin series is visible as a thin series.',
    ),
    array(
        'n'     => 'Module 03',
        ...
        'notes' => 'Multi-currency positions are reported both in local currency and in a single reporting currency, with the rate and date shown.',
    ),
    array(
        'n'     => 'Module 04',
        ...
        'notes' => 'Built for clients who need valuation activity to land inside their own credit or asset-management system rather than in an inbox.',
    ),
);
...
$__demo_rows = array(
    array('Bandra Kurla Complex, Mumbai', 'Grade-A Office', 'INR', '42.0 cr', '+6.2%', 'High',   88),
    array('Sector 16B, Noida',            'Grade-A Office', 'INR', '28.5 cr', '+3.8%', 'High',   81),
    array('Business Bay, Dubai',          'Residential',    'AED', '9.4 m',   '+5.1%', 'Medium', 64),
    array('Bhiwandi Corridor, MMR',       'Warehousing',    'INR', '19.2 cr', '+9.4%', 'Medium', 58),
    array('Al Reem Island, Abu Dhabi',    'Residential',    'AED', '6.1 m',   '-1.3%', 'Low',    37),
);
?>
<style>
...
<div class="vxn-plat">
    ...
    <section class="vxn-plat__intro">
        <div class="vxn-plat__wrap">
            <p class="vxn-plat__eyebrow">The platform</p>
            <h2 class="vxn-plat__h">What our technology actually does</h2>
            <p class="vxn-plat__lead">The VALUNXT Intelligence Platform is the software our own valuers, researchers and advisers work in. It is not a separate product line sold on claims about AI &mdash; it is the system that produces the valuations, market views and portfolio reporting behind our advisory work, opened up to clients who want the same view of their own holdings.</p>
            <p class="vxn-plat__lead" style="margin-top:16px;">Four modules, described below with what goes in, what comes out, and who it is for.</p>
        </div>
    </section>

    <section class="vxn-plat__ui">
        <div class="vxn-plat__wrap">
            <p class="vxn-plat__eyebrow">The screen</p>
            <h2 class="vxn-plat__h">The portfolio view</h2>
            <p class="vxn-plat__lead">Every holding in one place, valued on the same basis, with the confidence in each figure shown rather than implied. Assets that fall below the confidence threshold are flagged for a valuer instead of being reported as a number.</p>

            <figure class="vxn-plat__figure">
                <div class="vxn-plat__chrome">
                    <div class="vxn-plat__bar">
                        <span class="vxn-plat__dot" aria-hidden="true"></span>
                        <span class="vxn-plat__dot" aria-hidden="true"></span>
                        <span class="vxn-plat__dot" aria-hidden="true"></span>
                        <span class="vxn-plat__barname">VALUNXT Intelligence &mdash; Portfolio</span>
                        <span class="vxn-plat__barpill">Illustration</span>
                    </div>
                    <div class="vxn-plat__screen">
                        <div class="vxn-plat__kpis">
                            <div class="vxn-plat__kpi">
                                <p class="vxn-plat__kpilabel">Assets tracked</p>
                                <div class="vxn-plat__kpival">5</div>
                                <p class="vxn-plat__kpinote">Across 2 markets</p>
                            </div>
                            <div class="vxn-plat__kpi">
                                <p class="vxn-plat__kpilabel">Reporting currency</p>
                                <div class="vxn-plat__kpival">INR</div>
                                <p class="vxn-plat__kpinote">AED converted at close</p>
                            </div>
                            <div class="vxn-plat__kpi">
                                <p class="vxn-plat__kpilabel">Weighted movement</p>
                                <div class="vxn-plat__kpival vxn-plat__pos">+5.0%</div>
                                <p class="vxn-plat__kpinote">Trailing 12 months</p>
                            </div>
                            <div class="vxn-plat__kpi">
                                <p class="vxn-plat__kpilabel">Flagged for review</p>
                                <div class="vxn-plat__kpival">1</div>
                                <p class="vxn-plat__kpinote">Below confidence floor</p>
                            </div>
                        </div>

                        <div class="vxn-plat__tablewrap">
                            <table>
                                <caption>Holdings, indicative value and valuation confidence &mdash; sample data.</caption>
                                <thead>
                                    <tr>
                                ...
                                <tbody>
                                    <?php foreach ($__demo_rows as $__row): ?>
                                        <tr>
                                            <td><?= h($__row[0]) ?></td>
                                            <td><?= h($__row[1]) ?></td>
                                            <td><?= h($__row[2]) ?></td>
                                            <td><?= h($__row[3]) ?></td>
                                            <td class="<?= $__row[4][0] === '-' ? 'vxn-plat__neg' : 'vxn-plat__pos' ?>"><?= h($__row[4]) ?></td>
                                            <td>
                                                <span class="vxn-plat__conf">
                                                    <span class="vxn-plat__bar2" aria-hidden="true"><span style="width:<?= (int) $__row[6] ?>%"></span></span>
                                                    <?= h($__row[5]) ?>
                                                </span>
                                            </td>
                                        </tr>
                ...
                <figcaption class="vxn-plat__figcap"><strong>Illustration, not a screenshot.</strong> The layout above is the real portfolio screen; the holdings and figures are sample data used to show what the view contains. No client portfolio is depicted. Ask for a walkthrough on your own data and we will run the platform against your actual holdings.</figcaption>
            </figure>
        </div>
    </section>

    <section class="vxn-plat__mods">
        <div class="vxn-plat__wrap">
            <p class="vxn-plat__eyebrow">Modules</p>
            <h2 class="vxn-plat__h">Four modules, one data model</h2>
            <p class="vxn-plat__lead">Each module is usable on its own. They share one asset register, so a valuation produced in the first module is the number the third reports and the fourth delivers.</p>

            <div class="vxn-plat__modgrid">
                <?php foreach ($__modules as $__m): ?>
                    <article class="vxn-plat__mod">
                        <p class="vxn-plat__modn"><?= $__m['n'] ?></p>
                        <h3 class="vxn-plat__modt"><?= $__m['title'] ?></h3>
                        <p class="vxn-plat__modlede"><?= $__m['lede'] ?></p>
                        <dl class="vxn-plat__spec">
                            <dt>Inputs</dt><dd><?= $__m['in'] ?></dd>
                            <dt>Outputs</dt><dd><?= $__m['out'] ?></dd>
                            <dt>Used by</dt><dd><?= $__m['who'] ?></dd>
                        </dl>
                        <p class="vxn-plat__modnote"><?= $__m['notes'] ?></p>
                    </article>
                <?php endforeach; ?>
            </div>
        </div>
    ...
    <section class="vxn-plat__limits">
        <div class="vxn-plat__wrap">
            <p class="vxn-plat__eyebrow">Where it stops</p>
            <h2 class="vxn-plat__h">What the platform will not do</h2>
            <p class="vxn-plat__lead">Software that is only described by what it can do is hard to trust. These are the boundaries we hold it to.</p>
            <div class="vxn-plat__limitgrid">
                <div class="vxn-plat__limit">
                    <h3 class="vxn-plat__limitt">It does not replace a valuer</h3>
                    <p class="vxn-plat__limitd">Automated output is an input to a valuation opinion, not the opinion. Formal valuations are signed by a qualified valuer at <a href="<?= BASE ?>/our-group/reliant-surveyors/" style="color:var(--gd);">Reliant Surveyors</a>, who can and does override the model.</p>
                </div>
                <div class="vxn-plat__limit">
                    <h3 class="vxn-plat__limitt">It does not invent coverage</h3>
                    <p class="vxn-plat__limitd">Where comparable data is thin, the confidence band widens and the asset is flagged. The platform will report low confidence rather than produce a clean-looking number it cannot support.</p>
                </div>
                <div class="vxn-plat__limit">
                    <h3 class="vxn-plat__limitt">It does not give advice</h3>
                    <p class="vxn-plat__limitd">Outputs are analysis. Buy, hold and exit decisions sit with you, under an advisory mandate if you want our view on them. See the <a href="<?= BASE ?>/disclaimer/" style="color:var(--gd);">Disclaimer</a>.</p>
                </div>
            </div>
        </div>
    </section>

    <section class="vxn-plat__cta">
        <div class="vxn-plat__wrap">
            <p class="vxn-plat__eyebrow">Next step</p>
            <h2 class="vxn-plat__h">See it against your own portfolio</h2>
            <p class="vxn-plat__lead" style="margin:0 auto;">A walkthrough takes about forty minutes and runs on your holdings, not sample data. If the platform is not a fit for how you hold assets, we will tell you in that session.</p>
            <a class="vxn-plat__btn" href="<?= BASE ?>/contact/">Request a walkthrough</a>
        </div>
    </section>
