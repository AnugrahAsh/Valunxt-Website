...
/* Clients page — three alternating image/text advisory sections.
   ...
   alternating left/right, no CTA. Dummy content; images from new-folder. */
$__secs = array(
    array(
        ...
        'list'  => array('Bespoke portfolio construction', 'Confidential, conflict-free advice', 'Multi-generational wealth planning'),
    ),
    array(
        ...
        'list'  => array('Project funding & debt advisory', 'Equity & capital structuring', 'Institutional-grade due diligence'),
    ),
    array(
        ...
        'list'  => array('Cross border investment strategy', 'Regulatory & repatriation guidance', 'On-the-ground market intelligence'),
    ),
);
...
<section class="vxn-adv">
    <?php foreach ($__secs as $__i => $__s): $__rev = ($__i % 2 === 1); ?>
        <div class="vxn-adv__row<?= $__rev ? ' vxn-adv__row--reverse' : '' ?>">
            <div class="vxn-adv__media">
                <img class="vxn-adv__img" src="<?= BASE . h($__s['img']) ?>" alt="<?= h($__s['title']) ?>" loading="lazy">
                <div class="vxn-adv__bars" aria-hidden="true">
                    <?php foreach ($__bars as $__b): ?>
                        <span style="height:<?= (int)$__b ?>px"></span>
                    <?php endforeach; ?>
                </div>
            </div>
            <div class="vxn-adv__body">
                <span class="vxn-adv__badge"><?= h($__s['badge']) ?></span>
                <h2 class="vxn-adv__title"><?= h($__s['title']) ?></h2>
                <p class="vxn-adv__text"><?= h($__s['text']) ?></p>
                <ul class="vxn-adv__list">
                    <?php foreach ($__s['list'] as $__li): ?>
                        <li>
                            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" aria-hidden="true">
                                <circle cx="12" cy="12" r="10"></circle>
                                <path d="M8 12.5l2.6 2.6 5.4-6" stroke-linecap="round" stroke-linejoin="round"></path>
                            </svg>
                            <span><?= h($__li) ?></span>
                        </li>
                    <?php endforeach; ?>
