 ...
 * sidebar (author, share, topics, Talk to us, Related Insights).
 *
 * Expects in scope: $__r (site root), $PAGE, and $ARTICLE with keys:
 *   slug, title, category, hero_image, author, author_role, body (HTML)
 *   optional: read_time, topics (array), lead (bool handled by CSS)
 */
$__cat     = $ARTICLE['category'] ?? 'Insights';
$__topics  = $ARTICLE['topics'] ?? array($__cat);
$__author  = $ARTICLE['author'] ?? 'VALUNXT Research Team';
$__arole   = $ARTICLE['author_role'] ?? 'Insights & Analysis Desk';
$__initial = strtoupper(substr(trim($__author), 0, 1)) ?: 'V';
$__hero    = $ARTICLE['hero_image'] ?? ($PAGE['og_image'] ?? '');
$__wc      = str_word_count(strip_tags($ARTICLE['body'] ?? ''));
$__rt      = $ARTICLE['read_time'] ?? (max(1, (int) round($__wc / 200)) . ' min read');

$__origin  = ((!empty($_SERVER['HTTPS']) && $_SERVER['HTTPS'] !== 'off') ? 'https' : 'http') . '://' . ($_SERVER['HTTP_HOST'] ?? 'localhost');
$__url     = $__origin . BASE . ($PAGE['path'] ?? '/');
$__enc     = rawurlencode($__url);
$__shareLi = 'https://www.linkedin.com/sharing/share-offsite/?url=' . $__enc;
$__shareTw = 'https://twitter.com/intent/tweet?url=' . $__enc . '&text=' . rawurlencode($ARTICLE['title'] ?? '');
$__shareMl = 'mailto:?subject=' . rawurlencode($ARTICLE['title'] ?? '') . '&body=' . $__enc;

$__catalog = require $__r . '/includes/blog-catalog.php';
$__related = array();
foreach ($__catalog as $__slug => $__meta) {
    if ($__slug === ($ARTICLE['slug'] ?? '')) continue;
    $__related[$__slug] = $__meta;
}
$__related = array_slice($__related, 0, 3, true);
?>
<style>
...
<div id="main-content">
    <div id="main" role="main" class="vamtam-main layout-full">
        <article class="vxn-article">

            <header class="vxn-art-hero">
                <div class="vxn-art-hero__inner">
                    <div class="vxn-art-hero__text">
                        <div class="vxn-art-hero__meta">
                            <span class="vxn-badge">
                                <svg viewBox="0 0 512 512" aria-hidden="true"><path d="M362.7 19.3 492.7 149.3c25 25 25 65.5 0 90.5L293.3 439.2c-13.3 13.3-31.4 20.8-50.3 20.8H80c-26.5 0-48-21.5-48-48V240c0-19 7.5-37 20.8-50.3L272.2 19.3c25-25 65.5-25 90.5 0zM236.4 95.7 96 236.1V400h163.9L400.3 259.6 236.4 95.7z"/></svg>
                                Article
                            </span>
                            <span class="vxn-readtime"><?= h($__rt) ?></span>
                        </div>
                        <h1 class="vxn-art-hero__title"><?= h($ARTICLE['title']) ?></h1>
                        <div class="vxn-art-hero__eyebrow"><a href="<?= BASE ?>/blogs/">Insights</a> &nbsp;&rarr;&nbsp; <?= h(strtoupper($__cat)) ?></div>
                    </div>
                </div>
                <div class="vxn-art-hero__media" style="background-image:url('<?= BASE . h($__hero) ?>');"></div>
            </header>

            <div class="vxn-art-wrap">
                <div class="vxn-art-main">
                    <div class="vxn-art-content">
<?= $ARTICLE['body'] ?>
                    </div>
                </div>

                <aside class="vxn-art-side">
                    <?php if (!empty($ARTICLE['date'])): ?>
                        <div class="vxn-side-date">
                            <div class="vxn-side-label">Published</div>
                            <div class="vxn-side-date__val">
                                <time<?= !empty($ARTICLE['date_iso']) ? ' datetime="' . h($ARTICLE['date_iso']) . '"' : '' ?>><?= h($ARTICLE['date']) ?></time>
                            </div>
                        </div>
                    <?php endif; ?>
                    <div class="vxn-side-author">
                        <div class="vxn-avatar"><span><?= h($__initial) ?></span></div>
                        <div>
                            <div class="vxn-side-author__name"><?= h($__author) ?></div>
                            <div class="vxn-side-author__role"><?= h($__arole) ?></div>
                        </div>
                    </div>

                    <div class="vxn-side-block">
                        <div class="vxn-side-label">Share</div>
                        <div class="vxn-side-share">
                            <a href="<?= h($__shareLi) ?>" target="_blank" rel="noopener" aria-label="Share on LinkedIn">
                            ...
                            <a href="<?= h($__shareTw) ?>" target="_blank" rel="noopener" aria-label="Share on X">
                                <svg viewBox="0 0 512 512" aria-hidden="true"><path d="M389.2 48h70.6L305.6 224.2 487 464H345L233.7 318.6 106.5 464H35.8l164.9-188.5L26 48h145.6l100.5 132.9zm-24.8 373.8h39.1L151.1 88h-42z"/></svg>
                            </a>
                            <a href="<?= h($__shareMl) ?>" aria-label="Share by email">
                    ...
                    <div class="vxn-side-block">
                        <div class="vxn-side-label">Topics</div>
                        <div class="vxn-side-topics">
<?php foreach ($__topics as $__t): ?>
                            <a href="<?= BASE ?>/blogs/"><?= h($__t) ?></a>
<?php endforeach; ?>
                        </div>
                    </div>

                    <a class="vxn-side-cta" href="<?= BASE ?>/contact/">Talk to us</a>

<?php if ($__related): ?>
                    <div class="vxn-side-block" style="border-bottom:0;">
                        <div class="vxn-side-label">Related Insights</div>
                        <div class="vxn-side-related">
<?php foreach ($__related as $__slug => $__meta): ?>
                            <a class="vxn-rel" href="<?= BASE ?>/blogs/<?= h($__slug) ?>/">
                                <div class="vxn-rel__thumb">
                                    <span class="vxn-rel__badge">Article</span>
                                    <img src="<?= BASE . h($__meta['img']) ?>" alt="<?= h($__meta['title']) ?>" loading="lazy">
                                </div>
                                <div class="vxn-rel__title"><?= h($__meta['title']) ?></div>
                            </a>
<?php endforeach; ?>
                        </div>
                    </div>
