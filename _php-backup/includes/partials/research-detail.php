...
/* Research & Reports — single report DETAIL page body. Navy split hero
   ...
   Driven by $PAGE keys:
     ...
     crumbs (array label => href-relative-to-BASE, '' => not a link),
     topics (array of strings), intro (HTML), takeaways (array),
     pdf (href, '' => inert), sections (optional array of ['h'=>, 'body'=>HTML]).
   Self-contained CSS, scoped under .vxn-rd. */

$__type    = $PAGE['report_type'] ?? 'Report';
$__img     = $PAGE['hero_image'] ?? '';
$__title   = $PAGE['hero_title'] ?? ($PAGE['title'] ?? 'Report');
$__crumbs  = $PAGE['crumbs'] ?? array();
$__topics  = $PAGE['topics'] ?? array();
$__intro   = $PAGE['intro'] ?? '';
$__takes   = $PAGE['takeaways'] ?? array();
$__sections= $PAGE['sections'] ?? array();
...
/* Reading time is measured, not asserted. Every report page used to declare
   ...
   $PAGE['read_time'] explicitly (a downloadable PDF is longer than its summary
   page), but by default we count what the reader can see. */
$__read = trim((string) ($PAGE['read_time'] ?? ''));
if ($__read === '') {
    $__words = $__intro . ' ' . implode(' ', $__takes);
    foreach ($__sections as $__s) {
        $__words .= ' ' . ($__s['h'] ?? '') . ' ' . ($__s['body'] ?? '');
    }
    $__read = vxn_read_time($__words);
}
$__pdf     = $PAGE['pdf'] ?? '';
$__contact = BASE.'/contact/';

/* absolute URL for the share links */
$__scheme = (!empty($_SERVER['HTTPS']) && $_SERVER['HTTPS'] !== 'off') ? 'https' : 'http';
$__abs    = $__scheme.'://'.($_SERVER['HTTP_HOST'] ?? 'localhost').BASE.($PAGE['path'] ?? '/');
$__li     = 'https://www.linkedin.com/sharing/share-offsite/?url='.rawurlencode($__abs);
$__mail   = 'mailto:?subject='.rawurlencode($__title).'&body='.rawurlencode($__abs);
?>
<style>
...
<div id="main-content">
    <div id="main" role="main" class="vamtam-main layout-full">
        <article class="vxn-rd">

            <section class="vxn-rd__hero">
                <div class="vxn-rd__hero-inner">
                    <div class="vxn-rd__meta">
                        <span class="vxn-rd__badge">
                            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.6" aria-hidden="true"><path d="M12 6.5C10.5 5.3 8.5 4.8 6 4.8 4.6 4.8 3.4 5 2.5 5.3v13C3.4 18 4.6 17.8 6 17.8c2.5 0 4.5.5 6 1.7 1.5-1.2 3.5-1.7 6-1.7 1.4 0 2.6.2 3.5.5v-13c-.9-.3-2.1-.5-3.5-.5-2.5 0-4.5.5-6 1.7zM12 6.5v12.9" stroke-linecap="round" stroke-linejoin="round"/></svg>
                            <?= h($__type) ?>
                        </span>
                        <?php if ($__read !== ''): ?><span class="vxn-rd__read"><?= h($__read) ?></span><?php endif; ?>
                    </div>
                    <h1 class="vxn-rd__title"><?= $__title ?></h1>
                    <?php if ($__crumbs): ?>
                        <nav class="vxn-rd__crumbs" aria-label="Breadcrumb">
                            <?php $__n = 0; foreach ($__crumbs as $__lbl => $__href): ?>
                                <?php if ($__n++) echo '<span class="vxn-rd__arrow" aria-hidden="true">&rarr;</span>'; ?>
                                <?php if ($__href !== ''): ?>
                                    <a href="<?= BASE . h($__href) ?>"><?= h($__lbl) ?></a>
                                <?php else: ?>
                                    <span class="vxn-rd__cur"><?= h($__lbl) ?></span>
                                <?php endif; ?>
                            <?php endforeach; ?>
                        </nav>
                    <?php endif; ?>
                </div>
                <div class="vxn-rd__hero-media" style="background-image:url('<?= BASE . h($__img) ?>')"></div>
            </section>

            <section class="vxn-rd__body">
                <div class="vxn-rd__grid">
                    <div class="vxn-rd__main">
                        <?php if ($__intro !== ''): ?>
                            <p class="vxn-rd__intro"><?= $__intro ?></p>
                        <?php endif; ?>

                        <?php if ($__takes): ?>
                            <h2 class="vxn-rd__h">Key Takeaways</h2>
                            <ul class="vxn-rd__list">
                                <?php foreach ($__takes as $__t): ?>
                                    <li><?= h($__t) ?></li>
                                <?php endforeach; ?>
                            </ul>
                        <?php endif; ?>

                        <?php foreach ($__sections as $__s): ?>
                            <div class="vxn-rd__section">
                                <?php if (!empty($__s['h'])): ?><h2 class="vxn-rd__h"><?= h($__s['h']) ?></h2><?php endif; ?>
                                <?= $__s['body'] ?? '' ?>
                            </div>
                        <?php endforeach; ?>
                    </div>

                    <aside class="vxn-rd__side">
                        <div class="vxn-rd__siderow">
                            <span class="vxn-rd__sidelabel">Share</span>
                            <span class="vxn-rd__share">
                                <a href="<?= h($__li) ?>" target="_blank" rel="noopener" aria-label="Share on LinkedIn">
                                    <svg viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M4.98 3.5A2.5 2.5 0 1 0 5 8.5a2.5 2.5 0 0 0-.02-5zM3 9.75h4v11.25H3V9.75zM9.25 9.75H13v1.54h.05c.52-.94 1.8-1.93 3.7-1.93 3.96 0 4.7 2.4 4.7 5.53V21H17.7v-4.9c0-1.17-.02-2.67-1.7-2.67-1.7 0-1.96 1.27-1.96 2.58V21H9.25V9.75z"/></svg>
                                </a>
                                <a href="<?= h($__mail) ?>" aria-label="Share by email">
                                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.6" aria-hidden="true"><rect x="3" y="5" width="18" height="14" rx="2"/><path d="M3.5 6.5l8.5 6 8.5-6" stroke-linecap="round" stroke-linejoin="round"/></svg>
                                </a>
                            </span>
                        </div>
                        <?php if ($__topics): ?>
                            <div class="vxn-rd__siderow">
                                <span class="vxn-rd__sidelabel">Topics</span>
                                <span class="vxn-rd__topics"><?= h(implode(', ', $__topics)) ?></span>
                            </div>
                        <?php endif; ?>

                        <span class="vxn-rd__sidediv" aria-hidden="true"></span>

                        <a class="vxn-rd__btn vxn-rd__btn--solid" href="<?= $__pdf !== '' ? h($__pdf) : 'javascript:void(0)' ?>"<?= $__pdf !== '' ? ' download' : '' ?>>
                            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" aria-hidden="true"><path d="M12 3v11m0 0l-4-4m4 4l4-4M4 20h16" stroke-linecap="round" stroke-linejoin="round"/></svg>
                            <span>Download Report</span>
                        </a>
                        <a class="vxn-rd__btn vxn-rd__btn--ghost" href="<?= $__contact ?>">
                            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" aria-hidden="true"><rect x="3" y="5" width="18" height="14" rx="2"/><path d="M3.5 6.5l8.5 6 8.5-6" stroke-linecap="round" stroke-linejoin="round"/></svg>
