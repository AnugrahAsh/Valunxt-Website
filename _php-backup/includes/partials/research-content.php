<?php
/* Research & Reports page body — rebuilt to match the site's native case-study
   card UI (featured image on top, category pill + date meta row, serif title,
   excerpt) in a 2-column grid. Detail pages are not built yet, so the cards are
   intentionally NOT links (blank details, no redirect). 4 reports only.
   Images from new-folder. Self-contained CSS, scoped under .vxn-rr. */
...
<section class="vxn-rr">
    <div class="vxn-rr__wrap">
        <div class="vxn-rr__head">
            <p class="vxn-rr__eyebrow">Research &amp; Reports</p>
            <h2 class="vxn-rr__h">Intelligence that informs every decision</h2>
            <p class="vxn-rr__lead">Our research desk publishes independent reports on the markets we operate in — India and the UAE. Each report is grounded in primary data, on-the-ground intelligence, and the same discipline we bring to every client mandate.</p>
        </div>

        <div class="vxn-rr__grid">
            <?php foreach ($__reports as $__rp): ?>
                <article class="vxn-rr__card">
                    <span class="vxn-rr__media"><img src="<?= BASE . h($__rp['img']) ?>" alt="<?= h($__rp['title']) ?>" loading="lazy"></span>
                    <div class="vxn-rr__body">
                        <div class="vxn-rr__meta">
                            <span class="vxn-rr__pill"><?= h($__rp['tag']) ?></span>
                            <span class="vxn-rr__date"><?= h($__rp['date']) ?></span>
                        </div>
                        <h3 class="vxn-rr__title"><?= h($__rp['title']) ?></h3>
                        <p class="vxn-rr__desc"><?= h($__rp['desc']) ?></p>
                    </div>
                </article>
            <?php endforeach; ?>
        </div>

        <p class="vxn-rr__note">Detailed report pages are coming soon. For early access or a specific data request, please <a href="<?= BASE ?>/contact/">get in touch</a>.</p>
    </div>
</section>
